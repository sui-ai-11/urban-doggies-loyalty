...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get all active visits with timestamps
    var visits = [];
    var { data: vData, error: vErr } = await supabase
      ...
      .eq('status', 'active');
    if (vErr) {
      // Fallback without branch/staff columns
      var { data: vData2 } = await supabase
        .from('visits')
    ...
    var couponsAll = [];
    var { data: cpData, error: cpErr } = await supabase
      .from('coupons')
      .select('id, client_id, redeemed, branch, staff_name')
      .eq('business_id', businessID);
    if (cpErr) {
      var { data: cpData2 } = await supabase
        .from('coupons')
        .select('id, client_id, redeemed')
        .eq('business_id', businessID);
      couponsAll = cpData2 || [];
    } else {
      couponsAll = cpData || [];
    ...
    // Get all pets for breed info (may not exist for all tenants)
    var allPets = [];
    try {
      var { data: petData } = await supabase
        ...
        .eq('business_id', businessID);
      allPets = petData || [];
    } catch(e) { /* pets table may not exist */ }
